...
def visualization_data(terminal_id):
    """Get chart data for visualizations with numpy type handling"""
    print("🔥🔥🔥 ROOT VISUALIZATION FUNCTION CALLED! 🔥🔥🔥")
    logger.error("🔥🔥🔥 ROOT VISUALIZATION FUNCTION CALLED! 🔥🔥🔥")
    try:
        logger.info(f"🔥 ROOT CONTAINER VERSION - Getting chart data for terminal {terminal_id}")
        ...
        # Generate simple mock visualization data with type safety
        import random
        from datetime import datetime, timedelta
        
        base_time = datetime.now() - timedelta(days=7)
        
        # Historical cash levels (last 7 days) - ensure regular Python types
        historical_data = []
        for i in range(48):  # Last 48 hours
            ...
            historical_data.append({
                ...
                'cash_level': int(cash_level),  # Ensure regular int
                'dispensed_amount': int(random.randint(500, 3000))  # Ensure regular int
            })
        ...
        for i in range(7):
            ...
            daily_trend.append({
                'date': date,
                'average_cash': int(max(5000, avg_cash))  # Ensure regular int
            })
        ...
        for hour in range(24):
            ...
            usage_patterns[str(hour)] = {
                'average_dispensed': int(base_usage + random.randint(-200, 200)),  # Ensure regular int
                'transaction_count': int(random.randint(5, 25) if hour in peak_hours else random.randint(1, 8))  # Ensure regular int
            }
        ...
        for i in range(7):
            ...
            predictions.append({
                'date': future_date,
                'predicted_cash': int(predicted_cash),  # Ensure regular int
                'confidence': round(0.85 + random.uniform(-0.1, 0.1), 2)  # Regular float
            })
        ...
        response_data = {
            ...
            'current_status': {
                'cash_level': int(25000),  # Regular int
                'cash_percentage': float(50.0),  # Regular float
        ...
        logger.info("✅ Returning mock visualization data with type conversion")
        
        # Use custom JSON encoder to handle any remaining numpy types
        from flask import Response
        return Response(
            json.dumps(clean_response, cls=NumpyEncoder),
        ...
        error_response = {'error': f'Visualization data generation failed: {str(e)}'}
        return Response(
            json.dumps(error_response, cls=NumpyEncoder),
            mimetype='application/json',
            status=500
        )
