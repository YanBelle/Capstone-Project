...
print("ORIGINAL TEXT:")
print("Length: {} characters".format(len(sample_ej)))
print("Text: {}".format(repr(sample_ej)))
print("")
print("="*80)
print("")
...
print("1. After EJ header removal: {} chars".format(len(step1)))
print("   {}...".format(repr(step1[:100])))

# Step 2: Remove [020t patterns
step2 = re.sub(r'\[020t\s+', '', step1)
print("2. After [020t removal: {} chars".format(len(step2)))
print("   {}...".format(repr(step2[:100])))

# Step 3: Remove timestamps
step3 = re.sub(r'\s+\d{2}:\d{2}:\d{2}\s+', ' ', step2)
print("3. After timestamp removal: {} chars".format(len(step3)))
print("   {}...".format(repr(step3[:100])))

# Step 4: Remove transaction markers
step4 = re.sub(r'\s*---START OF TRANSACTION---\s*', ' ', step3)
print("4. After transaction marker removal: {} chars".format(len(step4)))
print("   {}...".format(repr(step4[:100])))

# Step 5: Replace *TRANSACTION START*
step5 = re.sub(r'\*TRANSACTION START\*', 'TRANSACTION_START', step4)
print("5. After *TRANSACTION START* fix: {} chars".format(len(step5)))
print("   {}...".format(repr(step5[:100])))

# Step 6: Replace PAN patterns
step6 = re.sub(r'PAN\s+\d{4}\d+\*+\d+', 'CardNumber', step5)
print("6. After PAN replacement: {} chars".format(len(step6)))
print("   {}...".format(repr(step6[:100])))

# Step 7: Remove complex transaction codes
step7 = re.sub(r'\*\d+\*\d+\*\([^,]+,\s*', '', step6)
print("7. After complex code removal: {} chars".format(len(step7)))
print("   {}...".format(repr(step7[:100])))
...
print("8. After compound token creation: {} chars".format(len(step8)))
print("   {}...".format(repr(step8[:100])))

# Step 9: Clean punctuation
step9 = re.sub(r'\*+', '_', step8)
step9 = re.sub(r'[(),]+', ' ', step9)
print("9. After punctuation cleaning: {} chars".format(len(step9)))
print("   {}...".format(repr(step9[:100])))

# Final cleanup
final_text = ' '.join(step9.split())
print("10. Final cleanup: {} chars".format(len(final_text)))
print("    {}".format(repr(final_text)))
...
print("FINAL RESULTS:")
print("Original length: {} characters".format(len(sample_ej)))
print("Processed length: {} characters".format(len(processed_text)))
reduction_pct = ((len(sample_ej) - len(processed_text)) / len(sample_ej)) * 100
print("Reduction: {:.1f}%".format(reduction_pct))
print("")
...
print("OK *TRANSACTION START* -> TRANSACTION_START")
print("OK PAN 0004263********1897 -> CardNumber") 
print("OK *7231*1*(Iw(1*3, M-02, R-10011 -> M-02 R-10011")
print("OK DEVICE ERROR -> DEVICE_ERROR (compound token)")
print("OK CARD INSERTED -> CARD_INSERTED (compound token)")
print("OK PIN ENTERED -> PIN_ENTERED (compound token)")
print("OK CARD TAKEN -> CARD_TAKEN (compound token)")
print("OK TRANSACTION END -> TRANSACTION_END (compound token)")
print("OK Reduced punctuation attention (*,(,) -> cleaned")
print("OK Preserved critical content: DEVICE_ERROR, ESC, VAL, REF, REJECTS")
...
for term in critical_terms:
    if term in processed_text:
        print("OK {} - PRESERVED".format(term))
    else:
        print("FAIL {} - MISSING".format(term))
