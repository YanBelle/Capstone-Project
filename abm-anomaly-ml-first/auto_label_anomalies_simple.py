...
def save_labels(labels):
    ...
    try:
        ...
        print("Successfully saved {} labels".format(result.get('saved_count', len(labels))))
        return True
    except requests.exceptions.RequestException as e:
...
def main():
    ...
    print("Found {} unlabeled anomalies".format(total_anomalies))
    ...
    for i in range(0, total_anomalies, BATCH_SIZE):
        ...
        print("\nProcessing batch {} ({} sessions)...".format(i//BATCH_SIZE + 1, len(batch)))
        ...
        for session in batch:
            ...
            print("  {}: {}".format(session_id[-12:], label))
        ...
        if save_labels(batch_labels):
            ...
            print("Failed to save batch {}".format(i//BATCH_SIZE + 1))
            break
    ...
    print("=" * 50)
    print("Successfully labeled: {}/{} anomalies".format(labeled_count, total_anomalies))
    
    if label_distribution:
        print("\nLabel Distribution:")
        for label, count in sorted(label_distribution.items(), key=lambda x: x[1], reverse=True):
            percentage = (count * 100.0) / labeled_count
            print("   - {}: {} ({:.1f}%)".format(label, count, percentage))
    
    if labeled_count == total_anomalies:
        print("\nAll anomalies have been labeled successfully!")
        print("You can now proceed to test supervised model training.")
    else:
        print("\n{} anomalies were not labeled due to errors.".format(total_anomalies - labeled_count))
