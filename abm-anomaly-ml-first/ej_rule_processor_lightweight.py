#!/usr/bin/env python3
"""
EJ Rule-Based Processor - Lightweight Version
==============================================

A rule-based solution using only Python standard library to:
1. Read EJ logs from ./data/input
2. Sessionize the logs 
3. Classify sessions as normal or error-containing
4. Store results in CSV format for easy access
...
class EJRuleBasedProcessor:
    """Lightweight rule-based processor for EJ logs"""
    ...
    def _save_results(self):
        ...
        self._save_sessions_to_csv(self.normal_sessions, normal_file)
        print(f"💾 Normal sessions saved to: {normal_file}")
        
        # Save error sessions
        error_file = self.output_dir / f"error_sessions_{timestamp}.csv"
        self._save_sessions_to_csv(self.error_sessions, error_file)
    ...
    def _save_sessions_to_csv(self, sessions: List[EJSession], filename: Path):
        """Save sessions to CSV using standard library"""
        if not sessions:
            # Create empty file
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as f:
            # Get fieldnames from first session
    ...
    def _save_session_summaries(self, timestamp: str):
        ...
        if self.normal_sessions:
            ...
            with open(normal_summary_file, 'w', newline='', encoding='utf-8') as f:
                fieldnames = ['session_id', 'file_source', 'start_time', 'transaction_type', 
                            'withdrawal_amount', 'card_inserted', 'pin_entered', 
                            'transaction_completed', 'notes_taken', 'card_taken']
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
        ...
        if self.error_sessions:
            ...
            with open(error_summary_file, 'w', newline='', encoding='utf-8') as f:
                fieldnames = ['session_id', 'file_source', 'start_time', 'error_types', 
                            'error_count', 'transaction_type', 'transaction_completed', 'severity']
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
